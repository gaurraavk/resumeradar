...
export async function register(input: { name: string; email: string; password: string; targetRole?: string }) {
  const db = await getDb(); const email = input.email.trim().toLowerCase();
  if (db.users.some(u => u.email === email)) throw new AppError('An account already exists for this email', 409, 'EMAIL_IN_USE');
  const now = new Date().toISOString(); const salt = randomBytes(16).toString('hex');
  const user: UserEntity = { id: ids.user(), name: input.name.trim(), email, passwordHash: hash(input.password, salt), salt, role: 'candidate', plan: 'Free Candidate Plan', targetRole: input.targetRole?.trim() || '', scansRemaining: 10, maxQuota: 10, status: 'Active', createdAt: now, updatedAt: now };
...
export async function login(emailInput: string, password: string, adminOnly = false) {
  const user = (await getDb()).users.find(u => u.email === emailInput.trim().toLowerCase());
  if (!user || !safeEqual(hash(password, user.salt), user.passwordHash)) throw new AppError('Invalid email or password', 401, 'INVALID_CREDENTIALS');
