...
export const TopNav: React.FC<TopNavProps> = ({
  ...
  return (
    <nav className="sticky top-0 z-50 flex flex-col w-full bg-[#faf9fe]/95 backdrop-blur-md border-b border-[#cfc4c5]/60">

      {/* Guest / Demo mode banner */}
      {user?.isGuest && (
        <div className="w-full bg-amber-400 text-amber-950 flex items-center justify-between px-4 sm:px-6 md:px-8 py-1.5 text-[11px] font-semibold">
          <span className="flex items-center gap-1.5">
            <span className="material-symbols-outlined text-[14px]">visibility</span>
            You are in <strong className="mx-1">Demo / Guest mode</strong> — sample data only, nothing is saved.
          </span>
          <button
            onClick={() => onNavigate('user-login')}
            className="ml-4 px-3 py-0.5 rounded-full bg-amber-950 text-amber-50 text-[10px] font-bold hover:bg-black transition-colors whitespace-nowrap"
          >
            Sign Up Free →
          </button>
        </div>
      ...
      {/* Main nav row */}
      <div className="flex justify-between items-center w-full px-4 sm:px-6 md:px-8 h-16">
        ...
        <div className="flex items-center gap-6 md:gap-8">
          ...
          <div className="hidden md:flex items-center gap-5 lg:gap-6">
            ...
            <button
              ...
              className={`text-xs lg:text-sm font-medium transition-colors px-1.5 py-1 cursor-pointer ${
                ['dashboard', 'new-analysis', 'analysis-result', 'optimization', 'cover-letter',
                 'ats-simulator', 'bullet-studio', 'interview-prep', 'template-studio'].includes(currentTab)
        ...
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          <div className="relative">
            <button
              onClick={() => { setNotificationsOpen(!notificationsOpen); setUserDropdownOpen(false); }}
            ...
            {notificationsOpen && (
              <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white border border-[#cfc4c5]/60 rounded-2xl shadow-xl p-4 z-50 text-sm space-y-3">
                <div className="flex justify-between items-center pb-2 border-b border-neutral-100">
                  <span className="font-bold text-xs text-black uppercase tracking-wider">Live Alerts & Job Radar</span>
                  <span className="text-[10px] text-[#0a66c2] font-semibold">{unreadCount} unread</span>
                </div>
                <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                  {notifications.length > 0 ? notifications.map((notif) => (
                    <div
                      key={notif.id}
                      onClick={() => { if (onMarkNotificationRead) onMarkNotificationRead(notif.id); }}
                      className={`p-3 rounded-xl border transition-colors cursor-pointer text-left ${
                        notif.read ? 'bg-neutral-50/60 border-neutral-200 opacity-80' : 'bg-blue-50/60 border-blue-200'
                      }`}
                    >
                      <div className="flex justify-between items-start gap-2">
                        <div className="flex items-center gap-1.5">
                          {notif.type === 'linkedin_match' ? (
                            <span className="w-5 h-5 rounded-md bg-[#0a66c2] text-white font-serif font-black text-[10px] flex items-center justify-center shrink-0">in</span>
                          ) : (
                            <span className="material-symbols-outlined text-[16px] text-[#0058bc]">auto_awesome</span>
                          )}
                      ...
                      <div className="flex justify-between items-center mt-2 pt-1.5 border-t border-neutral-200/50 text-[10px] text-[#7e7576]">
                        ...
                        {notif.linkedInJob && onOptimizeForJob && (
                          <button
                            onClick={(e) => { e.stopPropagation(); setNotificationsOpen(false); if (notif.linkedInJob) onOptimizeForJob(notif.linkedInJob); }}
                    ...
                    <div className="text-center py-6 text-xs text-[#7e7576]">No notifications right now.</div>
                  )}
                </div>
                <div className="pt-2 border-t border-neutral-100 flex justify-between items-center text-[11px]">
                  <button
                    onClick={() => { setNotificationsOpen(false); onNavigate('linkedin-radar'); }}
          ...
          {/* User / Guest / Sign-in area */}
          {user ? (
            user.isGuest ? (
              /* Guest chip */
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-1 rounded-full bg-amber-100 border border-amber-300 text-amber-800 text-[11px] font-bold tracking-wide">
                  DEMO
                </span>
                <button
                  onClick={() => onNavigate('user-login')}
                  className="flex items-center gap-1.5 text-xs font-semibold px-3.5 py-1.5 rounded-full bg-black text-white hover:bg-neutral-800 transition-all shadow-2xs cursor-pointer"
                >
                  Sign Up Free
                </button>
              </div>
              ...
              <div className="relative">
                ...
                >
                  <img
                    src={user.avatarUrl || 'https://lh3.googleusercontent.com/aida-public/AB6AXuCtGk6RIz7xVTQf8V6aPSiciReTIFq7WHfYpb2dvvScvF_xnaIgpX9TJl2p-L3xtxJR83Lt0Lk5uPxnglGGL7fA8MRtm2JGMjR9cWLAsxvFw833r-4hnxYuRyHVguG4oqkepEg0INPDriJPrxFRJ2LlxGbg7620QOv4OqszftGZRZ7deUSCESAZIiFySYxJZ-BS0r7d6_9_M1AjYl6TWJxDowsI0r6wVrFdYbxPQGIoZKFEqZdxoEtr6Q'}
                    alt={user.name}
                ...
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-72 bg-white border border-[#cfc4c5]/60 rounded-2xl shadow-xl p-4 z-50 text-xs space-y-3">
                    ...
                    <div className="space-y-1 pt-1">
                      <button
                        onClick={() => { setUserDropdownOpen(false); onNavigate('linkedin-radar'); }}
                      ...
                      <button
                        onClick={() => { setUserDropdownOpen(false); onNavigate('user-login'); }}
                        className="w-full flex items-center gap-2 p-2 hover:bg-[#faf9fe] rounded-lg text-[#4c4546] hover:text-black font-semibold cursor-pointer text-left"
                      >
                      ...
                      <button
                        onClick={() => { setUserDropdownOpen(false); if (onLogoutUser) onLogoutUser(); }}
